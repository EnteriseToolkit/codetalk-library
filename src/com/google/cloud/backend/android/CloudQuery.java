...
/**
 * A query object to execute it with {@link CloudBackend}. See {@link CloudBackendTest#testList()} for detailed usage.
 * 
 */
public class CloudQuery {
	...
	public enum Scope {
		/**
		 * PAST: query will be executed only for the entities that have been stored in past, and will not be executed
		 * when the entities will be updated in future.
		 */
		PAST,

		/**
		 * FUTURE: query will be executed only when entities are updated in future, and will not be executed against the
		 * current entities.
		 */
		FUTURE,

		/**
		 * FUTURE_AND_PAST: query will be executed for the current entities and future updates.
		 */
		FUTURE_AND_PAST
	}
	...
	/**
	 * Creates an instance of {@link CloudQuery} from the values of the specified CloudQuery. queryDto field will be
	 * deep copied, and filter field will be shallow copied.
	 * 
	...
	/**
	 * Sets the specified queryId to this query. This method will be used when developer want to control identity of
	 * each query explicitly, such as grouping multiple {@link CloudQuery} as one query and map it to one handler.
	 * 
	...
	/**
	 * Returns queryId of this query. Unless developer sets it by {@link #setQueryId(String)} explicitly, this method
	 * will return an ID based on the query condition. It means the same ID will be returned for multiple
	...
	private void setDefaultQueryIdIfNeeded() {
		// TODO: added an extra condition for uniqueness (sort property); do we need any more?
		if (this.queryDto.getQueryId() == null) {
			int queryHash = (gson.toJson(queryDto) + gson.toJson(filter) + gson
					.toJson(queryDto.getSortedPropertyName())).hashCode();
			String queryId = Base64.encodeToString(String.valueOf(queryHash).getBytes(), Base64.NO_PADDING
					| Base64.NO_WRAP);
			this.queryDto.setQueryId(queryId);
		}
	}
